...
from ArchMusic.Helpers.dossier import *
from ArchMusic.Helpers.inline import (
    buttons,
    close_key,
    help_back,
    helpmenu,
    ...
    if CallbackQuery.from_user.id != int(user_id):
        try:
            return await CallbackQuery.answer(
                "You're Not An Admin.", show_alert=True
            )
        except:
            return
...
async def admin_cbs(_, query: CallbackQuery):
    ...
    if data == "resume_cb":
        ...
        if not await is_streaming(query.message.chat.id):
            return await query.answer(
                "did you remember the resume Stream  ?", show_alert=True
            )
        await stream_off(query.message.chat.id)
        await pytgcalls.pause_stream(query.message.chat.id)
        await query.message.reply_text(
            text=f"streaming paused\n│ \nBY: {query.from_user.mention} ",
            reply_markup=close_key,
        )
        ...
        if not get:
            try:
                ...
                await pytgcalls.leave_group_call(query.message.chat.id)
                await query.message.reply_text(
                    text=f"stream skipped\n│ \nBY: {query.from_user.mention} \n\n**no more tracks in queue** {query.message.chat.title}, **leaving vc.**",
                    reply_markup=close_key,
                )
...
async def unban_ass(_, CallbackQuery):
    ...
    if umm.can_restrict_members:
        ...
        return await CallbackQuery.edit_message_text(
            f"{ASS_NAME} successfully unban by {CallbackQuery.from_user.mention}.\n\ntry to streaming something.."
        )
    else:
        return await CallbackQuery.answer(
            "give me ban rights for unban ",
            show_alert=True,
        )
